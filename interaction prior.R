########################################################
# Interraction prior construction
########################################################
# 
# started 20/03/2018
#
########################################################
interaction <- function(expertnb,combined=F, nSubg=3,marginal=F, CovSens=F, MeanSens=F,
  ...
  interaction <- paste("list(mutheta=c(",paste(a$p1,a$p2,a$p3,sep = ", "), "), ",
...
") # number of person at risk","
  ...
  write.table(interaction,quote = F,row.names  = F,col.names  = F,
              file="C:/Users/rt17164/Google Drive/Bristol/NIHR RMF/GAP Bayesian/GAP Subgroup/BUGS input/model 5 - CHARM with priors/interaction/data.txt")
  # return(cat(interaction, is.positive.definite(matrix(c(a$V11,a$V12,a$V13,
  #                                                       a$V12,a$V22,a$V23,
  #                                                       a$V13,a$V23,a$V33), byrow = T, nrow = 3))))
